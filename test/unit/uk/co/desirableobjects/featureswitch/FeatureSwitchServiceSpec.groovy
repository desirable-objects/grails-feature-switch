...
class FeatureSwitchServiceSpec extends Specification {
    ...
    def setup() {
        service.grailsApplication.config.features = [:]
    }

    @Unroll
    def "Returns enabled true/false depending on application config"() {
        given:
            service.grailsApplication.config.features[featureName] = [:]
            service.grailsApplication.config.features[featureName].enabled = featureEnabled
